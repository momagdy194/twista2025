...
Future<dynamic> myBackgroundMessageHandler(RemoteMessage message) async {
  // Make sure Flutter bindings are initialized for this isolate:
  WidgetsFlutterBinding.ensureInitialized();

  // Initialize Firebase for this isolate (required).
  try {
    await Firebase.initializeApp();
  } catch (e) {
    // ignore if already initialized or print for debugging
    debugPrint('Firebase init in background failed or already done: $e');
  }

  debugPrint("onBackground: ${message.data}");

  // Parse notification body safely without using Get or UI
  NotificationBodyModel notificationBody;
  try {
    notificationBody = NotificationHelper.convertNotification(message.data);
  } catch (e) {
    debugPrint('Failed to parse notification payload in background: $e');
    notificationBody = NotificationBodyModel(notificationType: NotificationType.general);
  }

  // Initialize a local instance of FlutterLocalNotificationsPlugin inside this isolate
  ...
  try {
    ...
    debugPrint('fln.initialize error in background: $e');
  }

  // If it's an order notification, start your foreground service (which will handle audio).
  if (notificationBody.notificationType == NotificationType.order) {
    try {
      // Initialize FlutterForegroundTask for this isolate (safe)
      FlutterForegroundTask.init(
        androidNotificationOptions: AndroidNotificationOptions(
          channelId: 'stackfood',
          channelName: 'Foreground Service Notification',
          channelDescription: 'Foreground service runs for new order.',
          onlyAlertOnce: false,
        ),
      ...
      // Start or restart the service — this runs in the app's foreground service isolate,
      // where your MyTaskHandler is executed and audio playback code should work.
      if (await FlutterForegroundTask.isRunningService) {
        await FlutterForegroundTask.restartService();
      } else {
        await FlutterForegroundTask.startService(
          serviceId: 256,
          notificationTitle: 'You got a new order (${notificationBody.orderId ?? ''})',
          notificationText: 'Open app and check order details.',
          callback: startCallback,
        );
      }
    } catch (e) {
      debugPrint('Failed to start foreground service from background handler: $e');
      // As fallback, show a local notification:
      await _showLocalFallbackNotification(fln, message);
    }
  } else {
    // For non-order notifications: show a local notification using our fresh plugin instance
    await _showLocalFallbackNotification(fln, message);
  }
...
Future<void> _showLocalFallbackNotification(FlutterLocalNotificationsPlugin fln, RemoteMessage message) async {
  try {
    ...
    const AndroidNotificationDetails androidDetails = AndroidNotificationDetails(
      '6ammart', // channel id
      AppConstants.appName, // channel name
    ...
    await fln.show(0, title, body, details, payload: jsonEncode(NotificationHelper.convertNotification(message.data).toJson()));
  } catch (e) {
    debugPrint('Error showing fallback local notification: $e');
  }
}
